...
import {
    Form, Input, Select, Button, Typography, Card, Tag, Space, App, Descriptions, Badge,
    Tabs, Empty, Spin, Row, Col,
} from 'antd';
import {
    CheckCircleOutlined, SafetyOutlined, UserAddOutlined, CloseOutlined, ReloadOutlined,
...
function AccessPanel({ apps, users }) {
    ...
    const loadRoles = useCallback(async (appId) => {
        if (!appId) return;
        setLoading(true);
        try {
    ...
    return (
        <div>
            <Space wrap style={{ marginBottom: 16 }}>
                <Text strong style={{ fontFamily: FONT }}>Sistema:</Text>
                <Select
                    style={{ minWidth: 280 }}
                ...
                />
                {selectedApp && (
                    <Button icon={<ReloadOutlined />} onClick={() => loadRoles(selectedApp)}>Actualizar</Button>
                )}
            </Space>

            {!selectedApp ? (
                <Empty description="Selecciona un sistema para gestionar sus roles y accesos" style={{ marginTop: 48 }} />
            ) : loading ? (
                    ...
                    {roles.map((role) => {
                        ...
                        return (
                            <Col xs={24} lg={12} key={role.id}>
                                ...
                                >
                                    {role.description && (
                                        <Text type="secondary" style={{ fontFamily: FONT, display: 'block', marginBottom: 12 }}>
                                            {role.description}
                                        </Text>
                                    )}
                                    ...
                                    <div style={{ margin: '8px 0 16px' }}>
                                        {assigned.length === 0 ? (
                                            <Text type="secondary" style={{ fontSize: 12 }}>Sin usuarios asignados</Text>
                                        ) : (
                                            <Space size={[8, 8]} wrap>
                                                {assigned.map((u) => (
                                                    <Tag
                                                        ...
                                                        onClose={(e) => { e.preventDefault(); handleRemoveUser(role.id, u.id); }}
                                                        color={PURPLE}
                                    ...
                                    <Space.Compact style={{ width: '100%' }}>
                                        <Select
                                            ...
                                            onChange={(v) => setAddValue((prev) => ({ ...prev, [role.id]: v }))}
                                            filterOption={(input, option) => option?.label?.toLowerCase().includes(input.toLowerCase())}
                                            options={available.map((u) => ({ label: `${u.full_name} (${u.email})`, value: u.id }))}
                                        />
                                        <Button
                                            type="primary"
            ...
            <Col xs={24} lg={12}>
                <Card title="Verificar permiso" styles={{ header: { fontFamily: FONT } }}>
                    <Form form={checkForm} layout="vertical" onFinish={handleCheck}>
                        ...
                        <Form.Item name="application_slug" label="Sistema" rules={[{ required: true }]}>
                            <Select placeholder="Seleccionar sistema" options={apps.map((a) => ({ label: a.name, value: a.slug }))} />
                        </Form.Item>
                        <Form.Item name="permission" label="Permiso" rules={[{ required: true }]}>
                            <Input placeholder="Ej. minerva.users.manage" />
                        </Form.Item>
                        <Button type="primary" htmlType="submit" block icon={<CheckCircleOutlined />} style={{ borderRadius: 20 }}>
                            Verificar
                        </Button>
                    </Form>

                    {result && (
                        <Card size="small" style={{ marginTop: 16, background: result.allowed ? '#f6ffed' : '#fff2f0' }}>
                            <Space>
                                <Badge status={result.allowed ? 'success' : 'error'} />
                                <strong>{result.allowed ? 'PERMITIDO' : 'DENEGADO'}</strong>
                            </Space>
                            <p style={{ margin: '8px 0 0', fontSize: 12, color: '#8c8c8c' }}>{result.reason}</p>
                        </Card>
                    )}
                </Card>
            </Col>
            ...
            <Col xs={24} lg={12}>
                <Card title="Mis permisos por sistema" styles={{ header: { fontFamily: FONT } }}>
                    <Form form={meForm} layout="vertical" onFinish={handleMePerms}>
                        ...
                        <Button type="primary" htmlType="submit" block style={{ borderRadius: 20 }}>Consultar</Button>
                    </Form>

                    {myPerms && (
                        <div style={{ marginTop: 16 }}>
                            <Descriptions column={1} size="small" bordered>
                                <Descriptions.Item label="Usuario">{myPerms.user_id}</Descriptions.Item>
                                <Descriptions.Item label="Sistema">{myPerms.application_slug}</Descriptions.Item>
                                <Descriptions.Item label="Roles">
                                    {(myPerms.roles || []).map((r) => (
                                        <Tag key={r.id || r.slug || r} color={NAVY}>{r.name || r.slug || r}</Tag>
                                    ))}
                                </Descriptions.Item>
                                <Descriptions.Item label="Permisos">
                                    {(myPerms.permissions || []).map((p) => (
                                        <Tag key={p.id || p.slug || p} color={PURPLE}>{p.slug || p}</Tag>
                                    ))}
                                </Descriptions.Item>
                            </Descriptions>
                        </div>
...
export default function AuthorizationPage() {
    ...
    return (
        <div>
            <Title level={4} style={{ fontFamily: FONT }}>Autorización</Title>
            <Tabs items={items} />
        </div>
    );
}
