...
export default function DashboardPage() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const [profileRes, appsRes, usersRes, rolesRes, permsRes, groupsRes] = await Promise.all([
    ...
    const statCards = [
        ...
        { title: 'Aplicaciones', value: stats?.applications, icon: <AppstoreOutlined />, color: PURPLE },
    ...
    return (
        <div>
            ...
            <Row gutter={[16, 16]} style={{ marginTop: 8 }}>
                <Col xs={24} lg={8}>
                    <Card title="Mi cuenta" style={{ borderRadius: 12, height: '100%' }}
                ...
                <Col xs={24} lg={16}>
                    <Card title="Mis accesos por sistema" style={{ borderRadius: 12, height: '100%' }}
                        styles={{ header: { fontFamily: FONT } }}>
                        {systems.length === 0 ? (
                                ...
                                {systems.map((appId) => (
                                    <div key={appId}>
                                        ...
                                        <div style={{ marginTop: 6 }}>
                                            <Text type="secondary" style={{ fontFamily: FONT, fontSize: 12 }}>
                                                Roles:{' '}
                                            </Text>
                                            {bySystem[appId].roles.length === 0 ? (
                                                ...
                                                bySystem[appId].roles.map((r) => (
                                                    <Tag key={r.id} color={NAVY}>{r.name}</Tag>
                                                ))
                                            )}
                                        </div>
                                        <div style={{ marginTop: 6 }}>
                                            ...
                                            {bySystem[appId].permissions.length === 0 ? (
                                                ...
                                                bySystem[appId].permissions.map((p) => (
                                                    <Tag key={p.id} color={PURPLE}>{p.slug}</Tag>
                                                ))
                                            )}
                                        </div>
                                    </div>
